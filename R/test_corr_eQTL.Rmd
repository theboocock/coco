```{r, echo=T}
  ...
  corr = read.table("data/forCoco2", header=T)
 ...
 # freq_af$VAR = apply(dat$bed,2,function(x){var,na.rm=T})
  ...
  rsids =10:ncol(dos_data)
    ...
    gvars =c()
    for(j in rsids){
      m1 = summary(lm(scale(dos_data$ENSG00000137601,scale = F) ~ -1+ scale(dos_data[,j], scale = F)))
  ...
  freq_af$GVAR = gvars
   ...
   res_preparation = prep_dataset_common(data_set = freq_af,ld_matrix= ld_matrix,hwe_variance = F,var_y=0.01943165)
